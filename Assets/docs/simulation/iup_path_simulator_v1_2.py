...
# ── PHASE 5: TIMEOUT STRESS TEST ─────────────────────────────────────────────
# AD-33: Validates that the timeout penalty (Stability −X, Confidence −X per turn)
# combined with maximum between-turn drift cannot cross a consequence threshold
# without a decision-layer action contributing.
# If the check fails, reduce stability_delta / public_confidence_delta in data.js.

TIMEOUT_STABILITY_DELTA  = -3   # reduced from -5 after this check (see comments below)
TIMEOUT_CONFIDENCE_DELTA = -3
TURNS_COUNT              = 6


def run_timeout_stress_test():
    """
    Worst-case: timeout fires every turn (6 × penalty) PLUS maximum between-turn drift.
    Starting state is INITIAL (no decision-layer actions).
    Confirm no consequence threshold is crossed by external modifiers alone.
    """
    print("\n" + "=" * 60)
    print("TIMEOUT + DRIFT STRESS TEST (AD-33 / Phase 5)")
    print("=" * 60)

    # Maximum per-variable drift from between-turn events (worst-case draw)
    # Phoebe pool (3 drawn): worst for each variable
    # stability:  EVT-P02 -2, EVT-P04 -2  → max loss = -4
    # resources:  EVT-P01 -3               → max loss = -3
    # confidence: EVT-P05 -2               → max loss = -2
    # workload:   EVT-P03 +3               → max gain = +3
    worst_phoebe = [
        {"id": "EVT-P02", "variable": "stability",  "effect": -2},
        {"id": "EVT-P04", "variable": "stability",  "effect": -2},
        {"id": "EVT-P01", "variable": "resources",  "effect": -3},
    ]
    # Cassandra pool (2 drawn): worst for each variable
    # stability:  EVT-C01 -4               → max loss = -4
    # confidence: EVT-C03 -4               → max loss = -4
    worst_cassandra = [
        {"id": "EVT-C01", "variable": "stability",  "effect": -4},
        {"id": "EVT-C03", "variable": "confidence", "effect": -4},
    ]
    ...
    print(f"\nTimeout penalty per turn: stability {TIMEOUT_STABILITY_DELTA:+d}, "
          f"confidence {TIMEOUT_CONFIDENCE_DELTA:+d}")
    print(f"Number of timeouts:       {TURNS_COUNT}")
    print(f"\nWorst-case between-turn events:")
    for e in all_events:
        print(f"  {e['id']}: {e['variable']} {e['effect']:+d}")

    # Apply all 6 timeout penalties + all drift
    test_vars = dict(INITIAL)

    total_timeout_stability  = TURNS_COUNT * TIMEOUT_STABILITY_DELTA
    total_timeout_confidence = TURNS_COUNT * TIMEOUT_CONFIDENCE_DELTA
    test_vars["stability"]  = clamp(test_vars["stability"]  + total_timeout_stability)
    test_vars["confidence"] = clamp(test_vars["confidence"] + total_timeout_confidence)

    for event in all_events:
        var = event["variable"]
        test_vars[var] = clamp(test_vars[var] + event["effect"])

    print(f"\nVariable states after 6 timeouts + maximum drift:")
    for k, v in test_vars.items():
        print(f"  {k:<12}  start={INITIAL[k]}  end={v}  change={v - INITIAL[k]:+d}")
    ...
    for var, op, threshold, label in checks:
        val     = test_vars[var]
        crossed = (val <= threshold) if op == "<=" else (val >= threshold)
        status  = "FAIL — threshold crossed without player decision" if crossed else "PASS"
        if crossed:
            passed = False
        print(f"  {label}: {val} -> {status}")

    print(f"\nTimeout stress test result: {'PASS' if passed else 'FAIL'}")
    if not passed:
        print("  ACTION REQUIRED: Reduce TIMEOUT_STABILITY_DELTA / TIMEOUT_CONFIDENCE_DELTA")
        print("  in data.js and re-run before implementing AD-33.")
    else:
        print(f"  Min stability reachable (timeout+drift only): {test_vars['stability']}")
        print(f"  Min confidence reachable (timeout+drift only): {test_vars['confidence']}")
        print(f"  No consequence threshold crossed without player decision.")
    return passed
...
if __name__ == "__main__":
    ...
    if "--timeout-stress" in args:
        ok = run_timeout_stress_test()
